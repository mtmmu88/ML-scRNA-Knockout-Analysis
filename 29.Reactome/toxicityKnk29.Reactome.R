...
setwd("C:\\Users\\Administrator\\Desktop\\toxicityKnk\\29.Reactome")      #设置工作目录
...
#Reactome富集分析
kk <- enrichPathway(gene=gene, organism="human", pvalueCutoff=1, qvalueCutoff=1)
Reactome=as.data.frame(kk)
Reactome$geneID=as.character(sapply(Reactome$geneID,function(x)paste(rt$genes[match(strsplit(x,"/")[[1]],as.character(rt$entrezID))],collapse="/")))
Reactome=na.omit(Reactome)
Reactome=Reactome[(Reactome$pvalue<pvalueFilter & Reactome$p.adjust<p.adjustFilter),]
#保存显著富集的结果
write.table(Reactome, file="Reactome.txt", sep="\t", quote=F, row.names = F)

#定义展示通路的数目
showNum=30      #显示富集最显著的前30个通路
if(nrow(Reactome)<showNum){
	showNum=nrow(Reactome)
}
